...
function CreateClientDialog() {
  ...
  const create = useMutation({ mutationFn: () => gateway.createClient({ name: name.trim(), ...whenPresent("accessProfileId", profileId), ...whenPresent("approvalPolicyId", policyId), capabilities: [] }), onSuccess: (client) => { invalidate(keys.clients); setOpen(false); toast.success(`Created ${client.name}`) }, onError: (error: Error) => toast.error("Could not create client", { description: error.message }) })
