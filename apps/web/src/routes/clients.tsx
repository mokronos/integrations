...
function CreateClientDialog() {
  ...
  const create = useMutation({ mutationFn: () => gateway.createClient({ name: name.trim(), ...whenPresent("accessProfileId", profileId === "__default__" ? undefined : profileId), ...whenPresent("approvalPolicyId", policyId === "__default__" ? undefined : policyId), capabilities: [], approvalDelivery: { returnLink: true } }), onSuccess: (client) => { invalidate(keys.clients); setOpen(false); toast.success(`Created ${client.name}`) }, onError: (error: Error) => toast.error("Could not create client", { description: error.message }) })
