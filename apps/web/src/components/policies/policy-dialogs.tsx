...
export function ConfigurationDialog({ kind, source }: { readonly kind: "access-profile" | "approval-policy"; readonly source?: { readonly id: string; readonly name: string } }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger render={<Button variant={source === undefined ? "default" : "outline"} size={source === undefined ? "default" : "sm"} />}>{source === undefined ? <Plus className="size-4" /> : <Copy className="size-4" />}{source === undefined ? `New ${label}` : "Clone"}</DialogTrigger><DialogContent><DialogHeader><DialogTitle>{source === undefined ? `New ${label}` : `Clone ${source.name}`}</DialogTitle><DialogDescription>{source === undefined ? `Create a reusable ${label}.` : "Create an independent copy of this configuration."}</DialogDescription></DialogHeader><div className="space-y-1.5"><Label htmlFor={`${kind}-name`}>Name</Label><Input id={`${kind}-name`} value={name} onChange={(event) => setName(event.target.value)} /></div><DialogFooter><Button disabled={name.trim().length === 0 || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? "Saving..." : source === undefined ? "Create" : "Clone"}</Button></DialogFooter></DialogContent></Dialog>
}
