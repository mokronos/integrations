...
//
// Emitted from the grants held by the key that generated it, so this file is
// the caller's authorized surface — adding a tool here means adding a grant.
...
export const generateEffectModule = (
  ...
  const blocks = tools.map((entry) => {
    ...
    const approval = entry.decision === "require_approval"
      ? "\n * Calls to this tool are frozen for a human; the step retries until decided."
      : ""
