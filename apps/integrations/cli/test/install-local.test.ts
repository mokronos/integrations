...
describe("local CLI installer", () => {
  test("installs executable source shims for exactly i and ii", async () => {
    ...
    expect(agent).toContain(path.join(repositoryDirectory, "apps/integrations/cli/src/agent.ts"))
    expect(operator).toContain(path.join(repositoryDirectory, "apps/integrations/cli/src/main.ts"))
