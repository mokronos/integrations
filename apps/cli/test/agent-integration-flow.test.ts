...
const createTicket = integration({
  source: { kind: "executor", integration: ${JSON.stringify(createTicket.integration)}, tool: ${JSON.stringify(createTicket.name)} },
...
export const AgentAcceptance = defineWorkflow({
  ...
  run: function* (input, ctx) {
    ...
    expect(stored).toContain(createTicket.integration)
    expect(stored).toContain(createTicket.name)
    // The persisted definition names the tool but not the connection that served
    // it, so handing this file to someone whose connection is named differently
    // still resolves.
    expect(stored).not.toContain(createTicket.address)
    expect(stored).not.toContain(createTicket.connection)
