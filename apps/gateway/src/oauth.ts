...
> => {
  ...
  if (authorizationUrl === undefined || tokenUrl === undefined) {
    throw new Error("Could not discover OAuth authorization and token endpoints")
  }
...
export const startHostedAuthorization = async (
  input: AuthorizationRequest & { readonly publicUrl: string },
  auth: OAuthOperations
): Promise<HostedAuthorization> => {
  const options = Schema.decodeUnknownSync(AuthorizationRequest)(input)
...
export const authorizeInBrowser = async (
  input: AuthorizationRequest & {
  ...
  const completion = Promise.withResolvers<Connection>()
