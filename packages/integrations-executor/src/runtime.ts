...
import { Database, libsqlLayer, memoryLayer } from "./database.ts"
...
export const clientsLayer: Layer.Layer<McpHost | OpenApiInvoker> = Layer.mergeAll(
  McpHost.layer,
  OpenApiInvoker.layer
)

/** The host's own logic, over storage and clients supplied by a caller. */
export const capabilitiesLayer: Layer.Layer<
...
export type HostLayer = Layer.Layer<
...
/** An entirely in-memory host that still reaches the real network. Nothing is
 *  written, so two tests cannot see each other's catalog. */
export const testLayer: HostLayer = capabilitiesLayer.pipe(
  Layer.provideMerge(clientsLayer),
  Layer.provide(Layer.mergeAll(memoryLayer, CredentialStore.memoryLayer))
)
