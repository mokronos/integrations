...
import type { StoredTokens } from "./credentials.ts"
...
import { ExecutorOAuthProbe, ExecutorOwner } from "./schemas.ts"
...
export interface StartOptions {
  ...
  readonly clientOwner: ExecutorOwner
...
export class OAuthFlows extends Context.Service<
  OAuthFlows,
  {
    ...
    readonly probe: (url: string) => Effect.Effect<ExecutorOAuthProbe, OAuthError>
...
>()("@mokronos/integrations-executor/OAuthFlows") {
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const probe = Effect.fn("OAuthFlows.probe")(function* (url: string) {
        ...
        return yield* Schema.decodeUnknownEffect(ExecutorOAuthProbe)({
