...
export const isJsonNumber = (value: Json): value is number => Predicate.isNumber(value)

/** Whether a JSON value is a boolean. */
export const isJsonBoolean = (value: Json): value is boolean => Predicate.isBoolean(value)

/** Decodes a value produced outside this package — an `oas` projection, a
 *  driver row, an MCP payload — as JSON, or `None` when it is not representable:
 *  a cyclic object graph, a function, a `Date`.
 *
 *  This is the parse step the rest of the host relies on, so `unknown` here is
 *  the input being parsed rather than input left unparsed. */
// oxlint-disable-next-line anti-slop/no-unknown-parameters
export const asJsonOption = (value: unknown): Option.Option<Json> => decodeJson(value)

/** As {@link asJsonOption}, defaulting to `null`. Used where an unrepresentable
 *  value and an absent one mean the same thing to the caller. */
// oxlint-disable-next-line anti-slop/no-unknown-parameters
