...
import { isJsonObject, type Json, type JsonObject } from "./json.ts"
import { ExecutorMcpProbe } from "./schemas.ts"
...
export class McpHost extends Context.Service<
  McpHost,
  {
    ...
    readonly probe: (endpoint: string) => Effect.Effect<ExecutorMcpProbe, McpError>
    readonly listTools: (
      endpoint: string,
      credential: Option.Option<McpCredential>
...
>()("@mokronos/integrations-executor/McpHost") {
    ...
    Effect.sync(() => {
      ...
      const probe = Effect.fn("McpHost.probe")(function* (endpoint: string) {
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          return yield* Schema.decodeUnknownEffect(ExecutorMcpProbe)({
            connected: false,
