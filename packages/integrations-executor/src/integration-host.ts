...
import { connectionCredentialKey, CredentialStore } from "./credentials.ts"
import {
...
import { AuthTemplateSlug } from "./ids.ts"
import { McpHost } from "./mcp.ts"
import type { McpCredential } from "./mcp.ts"
...
import type { CompiledOperation } from "./openapi.ts"
...
import { normalizeOutputSchema, normalizeToolResult } from "./tool-result.ts"
...
): Effect.Effect<ExecutorIntegration, StorageError> =>
  Schema.decodeUnknownEffect(ExecutorIntegration)({
    slug: record.slug,
    name: record.name,
...
): Effect.Effect<ExecutorConnection, StorageError> =>
  Schema.decodeUnknownEffect(ExecutorConnection)({
    owner: record.owner,
    name: record.name,
...
export class IntegrationHost extends Context.Service<
  IntegrationHost,
  {
    readonly listIntegrations: () => Effect.Effect<
      ReadonlyArray<ExecutorIntegration>,
      StorageError
    >
    readonly findIntegration: (
      slug: IntegrationSlug
    ) => Effect.Effect<Option.Option<ExecutorIntegration>, StorageError>
    readonly detect: (
      url: string
    ) => Effect.Effect<ReadonlyArray<ExecutorDetection>, never>
    ...
    ) => Effect.Effect<ExecutorConnection, HostFailure>
    readonly listConnections: (
      filter?: { readonly integration?: IntegrationSlug; readonly owner?: ExecutorOwner }
    ) => Effect.Effect<ReadonlyArray<ExecutorConnection>, StorageError>
    ...
    readonly toolSummaries: (
      filter?: ToolFilter
    ) => Effect.Effect<ReadonlyArray<ExecutorToolSummary>, HostFailure>
    readonly listTools: (
      filter?: ToolFilter
    ) => Effect.Effect<ReadonlyArray<ExecutorTool>, HostFailure>
    readonly describeTool: (
      target: ExecutorToolAddress | ToolTarget
    ) => Effect.Effect<ExecutorTool, HostFailure>
    readonly execute: (
      address: ExecutorToolAddress,
      input: Json
    ) => Effect.Effect<Json, HostFailure>
  }
>()("@mokronos/integrations-executor/IntegrationHost") {
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const summaryOf = (options: {
        ...
        Schema.decodeUnknownEffect(ExecutorToolSummary)({
          address: toolAddress({
          ...
          if (integration.kind === "mcp") {
            ...
            return yield* Effect.forEach(definitions, (definition) =>
              Effect.gen(function* () {
                ...
                return yield* Schema.decodeUnknownEffect(ExecutorTool)({
                  ...summary,
      ...
      const toolSummaries = Effect.fn("IntegrationHost.toolSummaries")(
        function* (filter: ToolFilter = {}) {
          ...
          return tools.map((tool): ExecutorToolSummary => ({
      ...
      const describeTool = Effect.fn("IntegrationHost.describeTool")(
        function* (target: ExecutorToolAddress | ToolTarget) {
      ...
      const execute = Effect.fn("IntegrationHost.execute")(
        function* (address: ExecutorToolAddress, input: Json) {
      ...
      const detect = Effect.fn("IntegrationHost.detect")(function* (url: string) {
        ...
        const detections: Array<ExecutorDetection> = []
      ...
      return {
        ...
        findIntegration: Effect.fn("IntegrationHost.findIntegration")(
          function* (slug: IntegrationSlug) {
            const found = yield* store.findIntegration(slug)
            return yield* Option.match(found, {
              onNone: () => Effect.succeed(Option.none<ExecutorIntegration>()),
